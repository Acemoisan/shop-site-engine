...
2. **Intake.** Fill `docs/onboarding/<slug>-intake.md` from the kickoff input + any prior audit (pre-fill everything derivable; mark gaps). Capture the client's **preferred payment method** (default Interac e-Transfer; card link or cheque as fallback — see `docs/gtm/payment-and-terms.md`) — a normal field, **not** a `[BLOCKER]`. If any `[BLOCKER]` field is missing (owner email, public email, form-destination inbox, privacy-note decision, **and the domain/email-cutover fields: where the domain is registered, where email is hosted, whether the domain was bought through their old site platform, whether we're replacing a live site**), **batch them into ONE message to the operator**, then continue. The domain/email fields decide the DNS cutover method and surface email-MX + platform-lock risks early (see `deploy-shop-site` → "Domain cutover"). (Template: `docs/onboarding/client-intake.md`.)
...
7. **Deploy.** Per **`deploy-shop-site`**: standalone repo + host build hook + Storyblok publish webhook. **Deliver-first: the site goes live on OUR hosting/accounts so the client can see it; account/domain ownership does NOT transfer until payment clears at Gate 2** (see `docs/gtm/payment-and-terms.md`). Set everything up so transfer is a one-step invite/reset once paid.
8. **Handoff.** Write `docs/handoff/<slug>-handoff.md` from `docs/handoff/_handoff-template.md`: fill **§0 Payment** (amount, method from intake, invoice ref), every account + how the client takes ownership (reset/invite to their email — **never plaintext passwords**), all live URLs + edit links, how to edit/publish in Storyblok, how to swap placeholders.
9. **Commit** on a `feat/<slug>` branch.
10. **Gate 2 (payment-gated handoff).** Produce (a) the live preview URL the operator forwards so the client sees the finished site, and (b) a drafted **invoice** ($1,500 + add-ons, fields per `docs/gtm/payment-and-terms.md` §3). The **operator confirms payment cleared**, then runs the Delivery section of `docs/onboarding/operator-validation-checklist.md`. **Account/domain ownership transfer + final edit links are the LAST step, only after payment.** **Stop here** for the operator to validate, collect payment, and forward.
...
- **Deliverables are styled HTML, not just markdown.** Both the audit (`packages/audit/audit-<host>.html`) and the handoff (`docs/handoff/<slug>-handoff.html`) are **receipt-styled, Studio0rbit-branded** pages that open cleanly in a browser and forward as-is. Keep a markdown twin for editing; produce the HTML for the client.
  ...
  - Handoff HTML ← **auto-built at the end of a Deliver run.** Write a tiny FACTS file with the IDs the run already produced (`docs/handoff/<slug>.facts.json` — client, slug, `liveUrl`, `hostSite`, `host` (default `Cloudflare Pages`), `storyblokSpace`, `storeUrl`; copy `docs/handoff/_handoff.example.json`'s sibling `bitcoin-manor.facts.json`), then run **`node scripts/build-handoff.mjs docs/handoff/<slug>.facts.json`**. It **auto-derives the honest before→after PageSpeed** from the saved `packages/audit/audit-<host>.json` files (set `oldAuditHost`/`newAuditHost`), assembles the standard sections, and writes both `<slug>.handoff.json` (review/edit) + `<slug>-handoff.html` (send). Any handoff field added to FACTS overrides its generated default; the lower-level renderer is `scripts/render-handoff.mjs` (inline text supports `**bold**`, `[label](url)`, `` `code` ``).
