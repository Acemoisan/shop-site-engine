...
- **Images (CORRECTED):** their real **logo** (`head_alt.gif`, 650px) IS reusable and was used (nav + footer). Several **real product photos** (`master_panel`, `wall_plate1`, `wall_spkr`, `cpanel`) were reused too → WebP at native size in a "gear we install" band. Only the **cartoon clip-art** (CRT TV, camcorder, computer, remote) was rejected. ⚠️ Lesson: don't dismiss a legacy site's assets wholesale — *always pull the logo*, and check each image (their logo was a clean 650px mark, not clip-art). Remaining gap: their product originals are ~100px, so **higher-resolution photography** is the photo-sourcing add-on.
