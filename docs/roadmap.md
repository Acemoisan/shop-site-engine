...
## Progress log
- **2026-06-17:** 🔧 **Audit suite descoped — pruned the research-grounded machinery, kept the knowledge.** The `client-pipeline` orchestrator runs Audit mode on the existing `site-audit` + `packages/audit` collector, so the rubric/specialist layer (sub-projects #2–#6) had no consumer. Removed `src/foundation/*`, `src/specialists/*`, the generators, and the duplicate scorer path (collector's 38 tests stay green); kept the research docs in `docs/research/audit-foundation/`; folded the verified priorities + honesty rules into the `site-audit` skill. See [decisions.md](decisions.md) 2026-06-17.
